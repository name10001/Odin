"""
Ignore this!
This works however it is not ready yet and it is not in its final form
I put it in a separate file because git was being annoying
"""
...
class Deck:
    ...
    def get_card_weights(self, card_collection=None):
        """
    ...
    def get_card_weight(self, card, card_collection=None):
        if card_collection is not None:
            n_this_type = card_collection.number_of_type(card.CARD_TYPE)
            return card.CARD_FREQUENCY.get_frequency(len(card_collection), n_this_type)
        else:
    ...
    def add_random_cards_to(self, card_collection, number):
        ...
        """
        # calculate weights
        card_weights = self.get_card_weights(card_collection)
        for i in range(number):
            # add a random card based upon the weights
            card_index = random.choices(range(len(self.cards)), weights=card_weights)[0]
            card = self.cards[card_index]
            card_weights[card_index] = self.get_card_weight(card, card_collection)
            card_collection.add_card(card(self.game))
